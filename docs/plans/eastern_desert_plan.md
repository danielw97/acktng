...
**Vnum Range (fixed):** **1832-1931** (`V 1832 1931`) 
...
- `V 1832 1931` — full 100-vnum envelope; all rooms, mobs, and objects must stay within this range
...
### Act I - The Open Road and the False Safety (1832-1851)
...
- **Boss rooms** (1891, 1911, 1931): `room_flags` includes `no_mob` (`4`) per boss placement policy
- **Necropolis rooms** (Zone E, 1912-1931): `room_flags` includes `no_teleport` (`16384`) and `no_recall` (`8192`), except stairwell/transit rooms that allow recall for escape
- **Storm belt rooms** (Zone C, 1872-1891): `room_flags` includes `hot` (`32`) for desert heat hazard flavor
...
- All 100 room vnums (1832-1931) must be used; fill every available room-vnum slot.

### Zone A: Caravan Spine (1832-1851, 20 rooms)
...
- Key rooms:
 - **1832** New western entry gate from legacy route (important room, 5+ sentence description).
 - **1838** Charter Mile Marker (first branch hub; important room, 5+ sentence description).
 - **1844** Burned Convoy Basin (event hotspot).
 - **1851** Eastbound Split toward deep desert/pyramid approach (important room, 5+ sentence description).

### Zone B: Cistern Quarter (1852-1871, 20 rooms)
...
- Key rooms:
 - **1855** Moonwell Access Shaft (vertical connector via D4 up / D5 down exits).
 - **1860** The Ledger of Thirst (lore-rich archive room; `inside` sector; important room, 5+ sentence description with multiple `E` extra descriptions for inscriptions).
 - **1865** Synod Toll Court (elite patrol reset anchor).
 - **1871** Sealed Descent to buried observatory (important room, 5+ sentence description).

### Zone C: Sirocco Expanse (1872-1891, 20 rooms)
...
- Key rooms:
 - **1872** Stormfront Threshold (important room, 5+ sentence description).
 - **1879** Blueglass Dune Crest (visibility gimmick room; multiple `E` extra descriptions for what can be seen from the crest).
 - **1884** Carrion Banner Ridge (faction clash scene).
 - **1891** Eye of the Sirocco (mini-boss arena; `room_flags` includes `no_mob` per boss placement policy; important room, 5+ sentence description).

### Zone D: Obelisk Marches (1892-1911, 20 rooms)
...
- Key rooms:
 - **1893** The Twelve Obelisks (important room, 5+ sentence description with `E` extra descriptions for individual obelisk inscriptions).
 - **1900** Jackal Anointing Yard.
 - **1904** Cracked Star Dial Plaza (important room, 5+ sentence description).
 - **1911** Regent's Processional Gate (mini-boss room; `room_flags` includes `no_mob`; important room, 5+ sentence description; named exit `^processional gate` discoverable in description).

### Zone E: Buried Court Necropolis (1912-1931, 20 rooms)
...
- Exception: stairwell/transit rooms (1912, 1931) allow recall (omit `no_recall`) so players can escape.
- Includes final boss route and optional side-loot chambers.
- Key rooms:
 - **1912** Descending Funerary Ramp (zone entry; exempt from `no_recall`; important room, 5+ sentence description).
 - **1918** Chamber of Salt Oaths (important room, 5+ sentence description with `E` extra descriptions for oath inscriptions).
 - **1924** Eclipse Lens Gallery (arcane puzzle framing; important room with chained `E` extra descriptions for lens examination).
 - **1930** Throne of Dry Stars (final pre-boss; important room, 5+ sentence description).
 - **1931** The Regent Beneath the Ninth Sun (final boss room; `room_flags` includes `no_mob`; exempt from `no_recall`; important room, 5+ sentence description).
...
- **Branching paths replace loops**: the plan uses hub-and-spoke branching from 3 major hub rooms (Charter Mile Marker 1838, Stormfront Threshold 1872, Descending Funerary Ramp 1912) rather than circular loops. Players choose branches and backtrack through hubs.
...
- Mob vnums must be assigned in ascending sequential order with no gaps, starting from 1832.
- Keep and fully rewrite existing vnums **1832-1845**.
- Expand roster with additional desert population templates through contiguous sequential slots **1846-1863** (staying within the 1832-1931 vnum envelope).
- Target composition: ~32 mob templates (vnums 1832-1863).
...
- **Mini-boss 1 (1891, Zone C):** The Bannerless Reaver. Level 75. Melee profile. `act` adds `remember` (`8`). Items dropped must include `ITEM_BOSS` flag.
- **Mini-boss 2 (1911, Zone D):** High Anointer of the Synod. Level 85. Hybrid profile with defensive casting. Assigned `spec_cast_cleric` in `#SPECIALS`. Items dropped must include `ITEM_BOSS` flag.
- **Final Boss (1931, Zone E):** The Last Sand Regent. Level 100. Hybrid profile with strong offensive casting. `act` adds `undead` + `remember`. `affected_by`: `sanctuary` (`128`), `detect_invis` (`8`), `infrared` (`512`). Assigned `spec_cast_undead` in `#SPECIALS`. Items dropped must include `ITEM_BOSS` flag. Loot table (`l`/`L`) used for boss drops.
...
- Object vnums must be assigned in ascending sequential order with no gaps, starting from 1832.
- Fully replace current object concepts beginning at **1832**.
- Expand through contiguous sequential slots (suggested through **1886**) while staying within the 1832-1931 vnum envelope.
- Target composition: ~55 object templates (vnums 1832-1886).
...
- **Early (1832-1856):** Navigation pressure, bandit skirmish encounters, readable faction telegraphing through room descriptions and mob `long_descr`. Charter Caravaneers and low-level Reaver skirmishers. Level 30-50 content.
- **Mid (1857-1901):** Branching objectives, elite Synod ritual nodes, hazard traversal through hot storm rooms. Jackal Synod mid-tier and Reaver banner captains. Level 50-75 content.
- **Late (1902-1931):** Boss approach with strong lore density, undead court remnants, relic payoff. Ancient Court Remnants and all three bosses. Level 75-100 content.
...
- **Zone A (Caravan Spine):** Reset Charter Caravaneers at road rooms (outriders at 1834, 1840; quartermaster at 1838; water clerk at 1842). Reset Reaver skirmishers at ambush rooms (1836, 1846, 1848). Limit 1-2 per mob.
- **Zone B (Cistern Quarter):** Reset Synod initiates and toll-priests at shrine/cistern rooms (1854, 1860, 1865). Reset Blind Oracle at lore room (1860 or 1867). Limit 1 per mob.
- **Zone C (Sirocco Expanse):** Reset apex predators across storm rooms (glass scorpions, ash adders, dune stalkers at 1874, 1877, 1880, 1885). Reset Reaver banner captain at 1884. Reset mini-boss The Bannerless Reaver at 1891 (limit 1).
- **Zone D (Obelisk Marches):** Reset Synod elites at ritual nodes (dusk chanters at 1895, 1899; astromancer vizier at 1904). Reset oathbound sentinels at obelisk rooms (1893, 1897). Reset mini-boss High Anointer at 1911 (limit 1).
- **Zone E (Buried Court):** Reset Ancient Court Remnants throughout (salt mummies at 1914, 1920; eclipse wardens at 1924, 1928). Reset final boss The Last Sand Regent at 1931 (limit 1).

### Object Resets (`O` command)

Format: `O 0 <obj_vnum> <limit> <room_vnum> <notes>`

- Place lore objects (salt ledgers, oath tablets) in key lore rooms (1860, 1893, 1918) for atmospheric pickup.
...
1. **Zone B Sealed Descent (1871):** Direction down (D5) to Zone E entry.
  - Room exit `<locks>`: `door` (`1`). `<key_vnum>`: vnum of Cistern Vault Key object.
  - `D` reset: `D 0 1871 5 2` (locked).
  - Reverse exit on 1912 must also have `door` bit set.
  - Key object (Cistern Vault Key) must exist in `#OBJECTS`.

2. **Zone D Processional Gate (1911):** Direction south (D2) or appropriate direction to Zone E.
  - Room exit `<locks>`: `door` (`1`). `<key_vnum>`: vnum of Processional Gate Seal object.
  - `D` reset: `D 0 1911 2 2` (locked).
  - Reverse exit must also have `door` bit set.
  - Key object (Processional Gate Seal) must exist in `#OBJECTS`.

3. **Zone E Boss Chamber (1930 -> 1931):** Direction into boss room.
  - Room exit `<locks>`: `door` (`1`). `<key_vnum>`: vnum of Regent's Eclipse Key object.
  - `D` reset: `D 0 1930 <dir> 2` (locked).
  - Reverse exit on 1931 must also have `door` bit set.
...
- Gate final boss chamber (1931) with locked door requiring Regent's Eclipse Key, providing visible progression gating.
...
### Rooms
- [ ] Populate all 100 room vnums (1832-1931) in ascending sequential order with no gaps.
...
- [ ] Set `no_mob` on all boss rooms (1891, 1911, 1931).
- [ ] Set `no_teleport` + `no_recall` on Zone E rooms (except transit rooms 1912, 1931).
- [ ] Set `hot` on Zone C storm rooms (1872-1891).
...
### Mobiles
- [ ] Write all ~32 mob templates (vnums 1832-1863) in ascending sequential order with no gaps.
...
### Objects
- [ ] Write all ~55 object templates (vnums 1832-1886) in ascending sequential order with no gaps.
...
- [ ] Verify all vnums within `V 1832 1931` envelope.
