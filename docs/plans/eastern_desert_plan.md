...
**Vnum Range (fixed):** **2050-2149** (`V 2050 2149`) 
...
- `V 2050 2149` — full 100-vnum envelope; all rooms, mobs, and objects must stay within this range
...
### Act I - The Open Road and the False Safety (2050-2069)
...
- **Boss rooms** (2109, 2129, 2149): `room_flags` includes `no_mob` (`4`) per boss placement policy
- **Necropolis rooms** (Zone E, 2130-2149): `room_flags` includes `no_teleport` (`16384`) and `no_recall` (`8192`), except stairwell/transit rooms that allow recall for escape
- **Storm belt rooms** (Zone C, 2090-2109): `room_flags` includes `hot` (`32`) for desert heat hazard flavor
...
- All 100 room vnums (2050-2149) must be used; fill every available room-vnum slot.

### Zone A: Caravan Spine (2050-2069, 20 rooms)
...
- Key rooms:
 - **2050** New western entry gate from legacy route (important room, 5+ sentence description).
 - **2056** Charter Mile Marker (first branch hub; important room, 5+ sentence description).
 - **2062** Burned Convoy Basin (event hotspot).
 - **2069** Eastbound Split toward deep desert/pyramid approach (important room, 5+ sentence description).

### Zone B: Cistern Quarter (2070-2089, 20 rooms)
...
- Key rooms:
 - **2073** Moonwell Access Shaft (vertical connector via D4 up / D5 down exits).
 - **2078** The Ledger of Thirst (lore-rich archive room; `inside` sector; important room, 5+ sentence description with multiple `E` extra descriptions for inscriptions).
 - **2083** Synod Toll Court (elite patrol reset anchor).
 - **2089** Sealed Descent to buried observatory (important room, 5+ sentence description).

### Zone C: Sirocco Expanse (2090-2109, 20 rooms)
...
- Key rooms:
 - **2090** Stormfront Threshold (important room, 5+ sentence description).
 - **2097** Blueglass Dune Crest (visibility gimmick room; multiple `E` extra descriptions for what can be seen from the crest).
 - **2102** Carrion Banner Ridge (faction clash scene).
 - **2109** Eye of the Sirocco (mini-boss arena; `room_flags` includes `no_mob` per boss placement policy; important room, 5+ sentence description).

### Zone D: Obelisk Marches (2110-2129, 20 rooms)
...
- Key rooms:
 - **2111** The Twelve Obelisks (important room, 5+ sentence description with `E` extra descriptions for individual obelisk inscriptions).
 - **2118** Jackal Anointing Yard.
 - **2122** Cracked Star Dial Plaza (important room, 5+ sentence description).
 - **2129** Regent's Processional Gate (mini-boss room; `room_flags` includes `no_mob`; important room, 5+ sentence description; named exit `^processional gate` discoverable in description).

### Zone E: Buried Court Necropolis (2130-2149, 20 rooms)
...
- Exception: stairwell/transit rooms (2130, 2149) allow recall (omit `no_recall`) so players can escape.
- Includes final boss route and optional side-loot chambers.
- Key rooms:
 - **2130** Descending Funerary Ramp (zone entry; exempt from `no_recall`; important room, 5+ sentence description).
 - **2136** Chamber of Salt Oaths (important room, 5+ sentence description with `E` extra descriptions for oath inscriptions).
 - **2142** Eclipse Lens Gallery (arcane puzzle framing; important room with chained `E` extra descriptions for lens examination).
 - **2148** Throne of Dry Stars (final pre-boss; important room, 5+ sentence description).
 - **2149** The Regent Beneath the Ninth Sun (final boss room; `room_flags` includes `no_mob`; exempt from `no_recall`; important room, 5+ sentence description).
...
- **Branching paths replace loops**: the plan uses hub-and-spoke branching from 3 major hub rooms (Charter Mile Marker 2056, Stormfront Threshold 2090, Descending Funerary Ramp 2130) rather than circular loops. Players choose branches and backtrack through hubs.
...
- Mob vnums must be assigned in ascending sequential order with no gaps, starting from 2050.
- Keep and fully rewrite existing vnums **2050-2063**.
- Expand roster with additional desert population templates through contiguous sequential slots **2064-2081** (staying within the 2050-2149 vnum envelope).
- Target composition: ~32 mob templates (vnums 2050-2081).
...
- **Mini-boss 1 (2109, Zone C):** The Bannerless Reaver. Level 75. Melee profile. `act` adds `remember` (`8`). Items dropped must include `ITEM_BOSS` flag.
- **Mini-boss 2 (2129, Zone D):** High Anointer of the Synod. Level 85. Hybrid profile with defensive casting. Assigned `spec_cast_cleric` in `#SPECIALS`. Items dropped must include `ITEM_BOSS` flag.
- **Final Boss (2149, Zone E):** The Last Sand Regent. Level 100. Hybrid profile with strong offensive casting. `act` adds `undead` + `remember`. `affected_by`: `sanctuary` (`128`), `detect_invis` (`8`), `infrared` (`512`). Assigned `spec_cast_undead` in `#SPECIALS`. Items dropped must include `ITEM_BOSS` flag. Loot table (`l`/`L`) used for boss drops.
...
- Object vnums must be assigned in ascending sequential order with no gaps, starting from 2050.
- Fully replace current object concepts beginning at **2050**.
- Expand through contiguous sequential slots (suggested through **2104**) while staying within the 2050-2149 vnum envelope.
- Target composition: ~55 object templates (vnums 2050-2104).
...
- **Early (2050-2074):** Navigation pressure, bandit skirmish encounters, readable faction telegraphing through room descriptions and mob `long_descr`. Charter Caravaneers and low-level Reaver skirmishers. Level 30-50 content.
- **Mid (2075-2119):** Branching objectives, elite Synod ritual nodes, hazard traversal through hot storm rooms. Jackal Synod mid-tier and Reaver banner captains. Level 50-75 content.
- **Late (2120-2149):** Boss approach with strong lore density, undead court remnants, relic payoff. Ancient Court Remnants and all three bosses. Level 75-100 content.
...
- **Zone A (Caravan Spine):** Reset Charter Caravaneers at road rooms (outriders at 2052, 2058; quartermaster at 2056; water clerk at 2060). Reset Reaver skirmishers at ambush rooms (2054, 2064, 2066). Limit 1-2 per mob.
- **Zone B (Cistern Quarter):** Reset Synod initiates and toll-priests at shrine/cistern rooms (2072, 2078, 2083). Reset Blind Oracle at lore room (2078 or 2085). Limit 1 per mob.
- **Zone C (Sirocco Expanse):** Reset apex predators across storm rooms (glass scorpions, ash adders, dune stalkers at 2092, 2095, 2098, 2103). Reset Reaver banner captain at 2102. Reset mini-boss The Bannerless Reaver at 2109 (limit 1).
- **Zone D (Obelisk Marches):** Reset Synod elites at ritual nodes (dusk chanters at 2113, 2117; astromancer vizier at 2122). Reset oathbound sentinels at obelisk rooms (2111, 2115). Reset mini-boss High Anointer at 2129 (limit 1).
- **Zone E (Buried Court):** Reset Ancient Court Remnants throughout (salt mummies at 2132, 2138; eclipse wardens at 2142, 2146). Reset final boss The Last Sand Regent at 2149 (limit 1).

### Object Resets (`O` command)

Format: `O 0 <obj_vnum> <limit> <room_vnum> <notes>`

- Place lore objects (salt ledgers, oath tablets) in key lore rooms (2078, 2111, 2136) for atmospheric pickup.
...
1. **Zone B Sealed Descent (2089):** Direction down (D5) to Zone E entry.
  - Room exit `<locks>`: `door` (`1`). `<key_vnum>`: vnum of Cistern Vault Key object.
  - `D` reset: `D 0 2089 5 2` (locked).
  - Reverse exit on 2130 must also have `door` bit set.
  - Key object (Cistern Vault Key) must exist in `#OBJECTS`.

2. **Zone D Processional Gate (2129):** Direction south (D2) or appropriate direction to Zone E.
  - Room exit `<locks>`: `door` (`1`). `<key_vnum>`: vnum of Processional Gate Seal object.
  - `D` reset: `D 0 2129 2 2` (locked).
  - Reverse exit must also have `door` bit set.
  - Key object (Processional Gate Seal) must exist in `#OBJECTS`.

3. **Zone E Boss Chamber (2148 -> 2149):** Direction into boss room.
  - Room exit `<locks>`: `door` (`1`). `<key_vnum>`: vnum of Regent's Eclipse Key object.
  - `D` reset: `D 0 2148 <dir> 2` (locked).
  - Reverse exit on 2149 must also have `door` bit set.
...
- Gate final boss chamber (2149) with locked door requiring Regent's Eclipse Key, providing visible progression gating.
...
### Rooms
- [ ] Populate all 100 room vnums (2050-2149) in ascending sequential order with no gaps.
...
- [ ] Set `no_mob` on all boss rooms (2109, 2129, 2149).
- [ ] Set `no_teleport` + `no_recall` on Zone E rooms (except transit rooms 2130, 2149).
- [ ] Set `hot` on Zone C storm rooms (2090-2109).
...
### Mobiles
- [ ] Write all ~32 mob templates (vnums 2050-2081) in ascending sequential order with no gaps.
...
### Objects
- [ ] Write all ~55 object templates (vnums 2050-2104) in ascending sequential order with no gaps.
...
- [ ] Verify all vnums within `V 2050 2149` envelope.
