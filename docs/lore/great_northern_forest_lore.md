# Lore: Great Northern Forest

Extracted from `great_northern_forest_plan.md`.
...
- **Thornwood exclusion zone:** wardens classify the Thornwood estate-forest as a southern fringe anomaly linked to grave-route tunnels and Shadowmere's approach corridors. It is technically within the broader northern woodland belt, but excluded from ordinary Lantern Road timber and escort contracts due to necrotic contamination and unstable path behavior. The site was originally a burial woodland used by ridge clans related to the Cairn-Keepers and Mire-Speakers — southern-fringe communities that practiced face-down oathbreaker burials, thorn-ring containment growth, and ritualized path detours around ancestor stands. These peoples were geographically separated from the main Lantern Road corridor but shared the same belief that the landscape itself participated in burial adjudication. When the Harren Charter established a fortified manor on the site during Midgaard's Founding Generation, these customs were displaced but not erased — they were later reinterpreted by the Root Covenant cult that emerged among the estate's grave laborers.

Beneath the gravewood lies a crystal substrate of unknown origin that predates human settlement. Whether it is a natural geological anomaly, a remnant of the same pre-human construction tradition attested by the Watchers' Cairn fragments at Midgaard's founding site, or a deposit related to the Spirebound Conclave's Prism Thesis crystal experiments (which produced the Withered Depths blight elsewhere in the forest) is unresolved. What is known is that the substrate is responsive: it mediates the necrotic ecology that makes Thornwood dangerous, and the binding anchored in its crystal heart is currently failing. The Wild Hunt — Thornwood's mobile enforcement arm — has extended its patrol range farther toward the Lantern Road's southern approaches than in any prior generation.

Warden cartographers mark Thornwood with a non-standard hazard sigil rather than a normal district tint, and convoy writs treat its perimeter as a legal exception area: route promises remain valid around it, but never through it. This administrative carve-out is politically useful because it lets both Midgaard and Kowloon acknowledge Thornwood as real while denying full jurisdictional responsibility for what happens inside. The carve-out also means no warden patrol is responsible for monitoring the binding's status — a gap that may prove consequential if the failure accelerates.
