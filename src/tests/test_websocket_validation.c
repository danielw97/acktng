...
static bool validate_websocket_handshake_request(const char *request, char *key_out,
                                                 size_t key_out_size, size_t *header_len_out)
{
...
static void test_rejects_non_get_or_incomplete(void)
{
   ...
   assert(!validate_websocket_handshake_request("GET / HTTP/1.1\r\nHost: local\r\n", key,
                                                sizeof(key), &header_len));
}

static void test_rejects_missing_required_headers(void)
{
   char key[64];
   const char *missing_upgrade = "GET / HTTP/1.1\r\n"
                                 "Host: localhost\r\n"
                                 "Sec-WebSocket-Key: abc123==\r\n"
                                 "\r\n";
   const char *missing_key = "GET / HTTP/1.1\r\n"
                             "Host: localhost\r\n"
                             "Upgrade: websocket\r\n"
                             "\r\n";
