...
static void test_resolve_persistent_room_uses_requested_room_when_present(void)
{
   ...
   assert(resolve_persistent_container_room_vnum_for_test(requested_room.vnum) ==
          requested_room.vnum);
}
...
{
   assert(persistent_container_where_vnum_for_save_for_test(4567, 1) == 65323);
}

static void test_keep_chest_corpse_save_uses_default_when_room_missing(void)
{
   assert(persistent_container_where_vnum_for_save_for_test(0, 0) == 65323);
}
...
static void test_link(struct fread_corpse_test_node *link, struct fread_corpse_test_node **first,
...
static void test_unlink(struct fread_corpse_test_node *link, struct fread_corpse_test_node **first,
                        struct fread_corpse_test_node **last)
{
   if (!link->next)
      *last = link->prev;
...
static size_t write_minimal_chest_to_buf(int vnum, const char *name, int nest, char *out,
                                         size_t out_size)
{
   FILE *fp = tmpfile();
   size_t n;
...
static void test_quest_static_done_cap_clears_slots_above_saved_cap(void)
{
   int count_three = quest_static_done_cap_true_count_for_test(3);
   int count_large = quest_static_done_cap_true_count_for_test(9999);
   assert(count_three <= 3);
   assert(count_three <= count_large);
}

static void test_quest_static_done_cap_clamps_negative_to_zero(void)
{
...
{
   int count_large = quest_static_done_cap_true_count_for_test(9999);
   int count_larger = quest_static_done_cap_true_count_for_test(10000);
