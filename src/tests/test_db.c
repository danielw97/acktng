...
static void fail_area_validation(const char *area_name, int area_line, const char *fmt, ...)
{
   ...
   fprintf(stderr, "test_db area validation failure in %s",
           area_name != NULL ? area_name : "<unknown area>");
   if (area_line > 0)
...
static void validate_vnum_in_area_range(const char *area_name, int area_line, int area_min_vnum,
                                        int area_max_vnum, int vnum, const char *context)
{
   if (vnum < area_min_vnum || vnum > area_max_vnum)
   {
      fail_area_validation(area_name, area_line, "%s vnum %d is outside assigned area range %d-%d",
                           context, vnum, area_min_vnum, area_max_vnum);
   }
}
...
static void validate_area_uses_only_assigned_vnums(FILE *fp, const char *area_name)
{
   ...
   while (fgets(line, sizeof(line), fp) != NULL)
   {
      ...
      if (section == AREA_SECTION_HEADER &&
          sscanf(trimmed, "V %d %d", &area_min_vnum, &parsed_max) == 2)
      {
         area_max_vnum = parsed_max;
         has_vnum_range = 1;
         if (area_min_vnum > area_max_vnum)
            fail_area_validation(area_name, current_area_line(fp), "invalid V vnum range %d-%d",
      ...
      if ((section == AREA_SECTION_ROOMS || section == AREA_SECTION_MOBILES ||
           section == AREA_SECTION_OBJECTS) &&
          sscanf(trimmed, "#%d", &vnum) == 1 && vnum != 0)
      {
         validate_vnum_in_area_range(area_name, current_area_line(fp), area_min_vnum, area_max_vnum,
                                     vnum, "section entry");
         continue;
      }
         ...
         if (trimmed[0] == 'D' && trimmed[1] >= '0' && trimmed[1] <= '5')
         {
            if (!consume_tilde_terminated_string(fp, 0) || !consume_tilde_terminated_string(fp, 0))
               fail_area_validation(area_name, current_area_line(fp),
                                    "unterminated exit description in #ROOMS entry");
            if (!read_non_blank_content_line(fp, line, sizeof(line)))
               fail_area_validation(area_name, current_area_line(fp),
                                    "unexpected EOF while reading room exit destination line");
            if (!is_exit_triple_line(skip_space(line)))
               fail_area_validation(area_name, current_area_line(fp),
                                    "invalid room exit destination line: %s", line);
         }
      }
   }

   fail_area_validation(area_name, current_area_line(fp),
                        "unterminated #ROOMS section (expected #0)");
}
...
static void validate_mobiles_section_format(FILE *fp, const char *area_name)
{
   ...
   while (read_non_empty_line(fp, line, sizeof(line)))
   {
      ...
      if (trimmed[0] != '#')
         fail_area_validation(area_name, current_area_line(fp),
                              "expected mobile entry vnum, got: %s", trimmed);
      ...
      if (!read_non_blank_content_line(fp, line, sizeof(line)) || strchr(line, 'S') == NULL)
         fail_area_validation(area_name, current_area_line(fp),
                              "mobile flags line missing expected 'S': %s", line);
      if (!read_non_blank_content_line(fp, line, sizeof(line)) ||
          !is_two_number_line(skip_space(line)))
         fail_area_validation(area_name, current_area_line(fp),
                              "mobile level/alignment line is invalid: %s", line);
      if (!read_non_blank_content_line(fp, line, sizeof(line)) ||
          !is_four_number_line(skip_space(line)))
         fail_area_validation(area_name, current_area_line(fp), "mobile combat line is invalid: %s",
                              line);
      ...
      for (;;)
      {
         ...
         if (trimmed[0] == '#')
         {
            if (fseek(fp, marker, SEEK_SET) != 0)
               fail_area_validation(area_name, current_area_line(fp),
                                    "unable to rewind to next mobile marker");
            break;
         }
         ...
         if (trimmed[0] == '!')
         {
            if (!has_prefixed_int_count(trimmed, '!', 7))
               fail_area_validation(area_name, current_area_line(fp), "invalid ! trailer line: %s",
                                    trimmed);
         }
         else if (trimmed[0] == '|')
         {
         ...
         else if (trimmed[0] == 'l')
         {
            if (!has_prefixed_int_count(trimmed, 'l', 10))
               fail_area_validation(area_name, current_area_line(fp), "invalid l trailer line: %s",
                                    trimmed);
         }
         else if (trimmed[0] == 'L')
         {
   ...
   fail_area_validation(area_name, current_area_line(fp),
                        "unterminated #MOBILES section (expected #0)");
}
      ...
      while (fgets(line, sizeof(line), area_fp) != NULL)
      {
         ...
         if (seen_line[vnum] != 0)
         {
            fail_area_validation(area_name, line_no,
                                 "duplicate index vnum %d in %s (first seen in %s line %d)", vnum,
