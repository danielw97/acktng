...
static void fail_area_test(const char *area_path, int line_number, const char *fmt, ...)
{
   ...
   fprintf(stderr, "area format test failure in %s",
           area_path != NULL ? area_path : "<unknown area>");
   if (line_number > 0)
...
static void add_unique_vnum(VNUM_NODE **set, int vnum, const char *area_path, int line_number,
                            const char *kind)
{
   if (vnum_set_contains(*set, vnum))
      fail_area_test(area_path, line_number, "duplicate %s vnum %d across loaded area files", kind,
                     vnum);

   vnum_set_add(set, vnum);
}

static void scan_area_indexes(const char *area_path, VNUM_NODE **global_room_vnums,
                              VNUM_NODE **global_mobile_vnums, VNUM_NODE **global_object_vnums)
{
...
static void parse_mobile_loot_extension(const char *line, int line_number, const char *area_path)
{
   ...
   if (trimmed[0] == 'l')
   {
      if (!parse_exact_n_ints(trimmed + 1, MAX_LOOT + 1))
         fail_area_test(area_path, line_number,
                        "mobile 'l' line must contain loot_amount plus %d loot vnums", MAX_LOOT);
      return;
   }
   ...
   if (trimmed[0] == 'L')
   {
      if (!parse_exact_n_ints(trimmed + 1, MAX_LOOT))
         fail_area_test(area_path, line_number,
                        "mobile 'L' line must contain exactly %d loot chance values", MAX_LOOT);
      return;
   }
}

static void validate_mobile_skill_level_bounds(int mob_level, int mob_skills, int line_number,
                                               const char *area_path)
{
   ...
   for (size_t i = 0; i < sizeof(floor_rules) / sizeof(floor_rules[0]); i++)
      if ((mob_skills & floor_rules[i].bit) != 0 && mob_level < floor_rules[i].min_level)
         fail_area_test(area_path, line_number,
                        "mobile level %d is below minimum level %d for skill '%s'", mob_level,
                        floor_rules[i].min_level, floor_rules[i].name);

   if (mob_level < 50 && (mob_skills & (1 << 4)) != 0)
      fail_area_test(area_path, line_number,
                     "mobile level %d exceeds attack-pass ceiling (6_attack requires level 50+)",
                     mob_level);
}

static void require_inline_tilde_terminated_string(FILE *fp, char *line, int *line_number,
                                                   const char *area_path, const char *field_name)
{
   if (!read_line(fp, line, line_number))
      fail_area_test(area_path, *line_number, "unexpected EOF while reading mobile %s", field_name);

   if (strchr(line, '~') == NULL)
      fail_area_test(area_path, *line_number, "mobile %s must end with '~' on the same line",
                     field_name);
}
...
static void consume_tilde_string(FILE *fp, char *line, int *line_number, const char *area_path)
{
   do
   {
      if (!read_line(fp, line, line_number))
         fail_area_test(area_path, *line_number,
                        "unexpected EOF while reading ~-terminated string");
   ...
   while (read_line(fp, line, line_number))
   {
      ...
      if (trimmed[0] == 'Q')
      {
         ...
         if (parse_int_tokens(trimmed + 1, values, 1) != 1 || !parse_exact_n_ints(trimmed + 1, 1))
            fail_area_test(area_path, *line_number,
                           "#AREA 'Q' directive must be formatted as: Q <revision>");
         ...
         if (saw_required_q16)
            fail_area_test(area_path, *line_number,
                           "#AREA must contain exactly one 'Q 16' directive");
      ...
      else if (trimmed[0] == 'I' || trimmed[0] == 'V')
      {
         if (!parse_exact_n_ints(trimmed + 1, 2))
            fail_area_test(area_path, *line_number,
                           "#AREA '%c' directive must contain exactly 2 integers", trimmed[0]);
         ...
         if (trimmed[0] == 'V')
         {
            ...
            if (saw_v_directive)
               fail_area_test(area_path, *line_number,
                              "#AREA must contain exactly one 'V <min> <max>' directive");

            parse_int_tokens(trimmed + 1, values, 2);
            if (values[0] > values[1])
               fail_area_test(area_path, *line_number,
                              "#AREA 'V' directive min vnum must be <= max vnum");
   ...
   for (;;)
   {
      ...
      for (;;)
      {
         ...
         if (marker == '#')
         {
            ...
            (*line_number)--;
            validate_mobile_skill_level_bounds(
                mob_level, mob_skills, skill_line_number > 0 ? skill_line_number : *line_number,
                area_path);
            break;
         }
   ...
   for (;;)
   {
      ...
      for (;;)
      {
         ...
         if (trimmed[0] == 'A')
         {
            ...
            if (!read_line(fp, line, line_number) || !parse_exact_n_ints(line, 2))
               fail_area_test(area_path, *line_number,
                              "object affect must contain location and modifier");
         }
         else if (trimmed[0] == 'E')
         {
            consume_tilde_string(fp, line, line_number, area_path);
         ...
         else
         {
            ...
            if (!read_line(fp, line, line_number) || !parse_exact_n_ints(line, 1))
               fail_area_test(area_path, *line_number,
                              "object level marker must be followed by integer level");
         }
      }
   }
}

static void consume_exit_keyword_string(FILE *fp, char *line, int *line_number,
                                        const char *area_path)
{
   for (;;)
   {
      ...
      if ((trimmed[0] == 'D' && trimmed[1] >= '0' && trimmed[1] <= '5') || trimmed[0] == 'E' ||
          trimmed[0] == 'S' || trimmed[0] == '#')
         fail_area_test(area_path, *line_number,
                        "room exit keyword missing '~' terminator before room token");

      if (parse_exact_n_ints(trimmed, 2) || parse_exact_n_ints(trimmed, 3))
         fail_area_test(area_path, *line_number,
                        "room exit keyword missing '~' terminator before numeric exit data");
      ...
      for (;;)
      {
         ...
         if (trimmed[0] == 'D')
         {
            ...
            if (!read_line(fp, line, line_number) || !parse_exact_n_ints(line, 3))
               fail_area_test(area_path, *line_number,
                              "exit destination line must contain exactly 3 integers");
...
static void parse_specials_section(FILE *fp, char *line, int *line_number, const char *area_path,
                                   const char *allowed_leads)
{
   for (;;)
   {
      const char *trimmed;
      ...
      if (!read_line(fp, line, line_number))
         fail_area_test(area_path, *line_number,
                        "unexpected EOF inside specials/object-functions section");
...
static void parse_resets_section(FILE *fp, char *line, int *line_number, const char *area_path,
                                 const VNUM_NODE *global_room_vnums, int area_min_vnum,
                                 int area_max_vnum)
{
   for (;;)
   {
...

      {
         ...
         if ((cmd == 'M' || cmd == 'O') && value_count >= 4)
         {
            ...
            if (room_vnum >= area_min_vnum && room_vnum <= area_max_vnum &&
                !vnum_set_contains(global_room_vnums, room_vnum))
               fail_area_test(area_path, *line_number, "reset '%c' references unknown room vnum %d",
                              cmd, room_vnum);
         }
      ...
      if (cmd == 'G' || cmd == 'R')
      {
         if (!parse_at_least_n_ints(trimmed + 1, 3))
            fail_area_test(area_path, *line_number,
                           "G/R resets must have command+ifflag+arg1+arg2 (4 integers total)");
      }
      else
      {
         char *end;
...
static void assert_area_matches_spec(const char *area_path, const VNUM_NODE *global_room_vnums,
                                     int area_min_vnum, int area_max_vnum)
{
   FILE *fp = fopen(area_path, "r");
   char line[LINE_MAX_LEN];
   int line_number = 0;
   ...
   while (read_line(fp, line, &line_number))
   {
      ...
      {
         parse_resets_section(fp, line, &line_number, area_path, global_room_vnums, area_min_vnum,
                              area_max_vnum);
         continue;
      }
      ...
      if (prev_min_vnum >= 0)
      {
             ...
             (area_min_vnum == prev_min_vnum && area_max_vnum < prev_max_vnum))
         {
            fail_area_test(
                ...
                area_name, area_min_vnum, area_max_vnum, prev_area_name, prev_min_vnum,
                prev_max_vnum);
         }
      }
