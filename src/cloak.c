...
bool cloak_handle_spell_defense(CHAR_DATA *ch, CHAR_DATA *victim, int dt, int element, int dam)
{
   if ((skill_table[dt].target == TAR_CHAR_OFFENSIVE) &&
       (IS_AFFECTED(victim, AFF_CLOAK_REFLECTION)) && (ch != victim) &&
       (number_percent() < (get_psuedo_level(victim) - 70)))
   {

      act("@@N$n's @@lc@@el@@ro@@ya@@ak@@N glows brightly as $Nn's spell hits it, and the spell is "
          "reflected@@N!!",
          ch, victim, NULL, TO_ROOM);
      act("@@N$N's @@lc@@el@@ro@@ya@@ak@@N glows brightly, and reflects your spell back on "
          "you@@N!!",
          ch, NULL, victim, TO_CHAR);
      act("@@NYour @@lc@@el@@ro@@ya@@ak@@N glows brightly, and reflects the spell back on $N@@N!!!",
   ...
   if ((skill_table[dt].target == TAR_CHAR_OFFENSIVE) &&
       (IS_AFFECTED(victim, AFF_CLOAK_ABSORPTION)) && (ch != victim) &&
       (number_percent() < (get_psuedo_level(victim) - 55)))
   {
      ...
      act("@@N$n's @@lcloak@@N glows brightly as $N's spell hits it, then fades@@N!!", victim, NULL,
          ch, TO_ROOM);
                                 ...
                                 bool shortfight_round)
{
       ...
       (ch != victim))
   {
      ...
      if (has_flaming && IS_SET(element, ELE_PHYSICAL))
      {
         if (!shortfight_round)
         {
            act("@@N$n's @@ecloak@@N flares and engulfs $N in @@eflames@@N!!", victim, NULL, ch,
                TO_NOTVICT);
         }
         else
         {
         ...
         if (!(shortfight_round && !IS_NPC(ch) && IS_SET(ch->config, CONFIG_SHORT_FIGHT)))
         {
            if (!IS_NPC(ch) && IS_SET(ch->pcdata->pflags, PFLAG_BLIND_PLAYER))
               ...
               act("@@N$N's @@ecloak@@N erupts and burns you in retaliation!!", ch, NULL, victim,
                   TO_CHAR);
         }
      ...
      else if (has_elements && !IS_SET(element, ELE_PHYSICAL) && !IS_SET(element, NO_REFLECT) &&
               !IS_SET(element, NO_ABSORB))
      {
         if (!shortfight_round)
         {
            act("@@N$n's @@lELEMENTS @@Ncloak crackles and lashes $N with raw power!!", victim,
                NULL, ch, TO_NOTVICT);
         }
         else
         {
            ...
            for (rch = ch->in_room->first_person; rch != NULL; rch = rch->next_in_room)
            {
               ...
               sprintf(
                   buf,
                   "@@N%s's @@lELEMENTS @@Ncloak crackles and lashes %s with raw power!!@@N\n\r",
                   PERS(victim, rch), PERS(ch, rch));
               send_to_char(buf, rch);
            }
         }
         ...
         if (!(shortfight_round && !IS_NPC(ch) && IS_SET(ch->config, CONFIG_SHORT_FIGHT)))
         {
            if (!IS_NPC(ch) && IS_SET(ch->pcdata->pflags, PFLAG_BLIND_PLAYER))
               ...
               act("@@N$N's @@lELEMENTS @@Ncloak crackles and shocks you in retaliation!!", ch,
                   NULL, victim, TO_CHAR);
         }
         ...
         if (!(shortfight_round && !IS_NPC(victim) && IS_SET(victim->config, CONFIG_SHORT_FIGHT)))
         {
            if (!IS_NPC(victim) && IS_SET(victim->pcdata->pflags, PFLAG_BLIND_PLAYER))
               ...
               act("@@NYour @@lELEMENTS @@Ncloak crackles and lashes back at $N!!!", victim, NULL,
                   ch, TO_CHAR);
         }
   ...
   if ((is_affected(victim, skill_lookup("cloak:misery"))) && (ch != victim) &&
       (number_range(0, 99) < 35))
   {
      AFFECT_DATA af;
      if (!shortfight_round)
      {
         act("@@N$n's @@dcloak@@N flares and shrouds $N in @@dmisery@@N!!", victim, NULL, ch,
             TO_NOTVICT);
      }
      else
      {
      ...
      if (!(shortfight_round && !IS_NPC(ch) && IS_SET(ch->config, CONFIG_SHORT_FIGHT)))
      {
         if (!IS_NPC(ch) && IS_SET(ch->pcdata->pflags, PFLAG_BLIND_PLAYER))
            ...
            act("@@N$N's @@dcloak@@N flares, and shrouds you with @@dmisery@@N!!", ch, NULL, victim,
                TO_CHAR);
      }
      ...
      if (!(shortfight_round && !IS_NPC(victim) && IS_SET(victim->config, CONFIG_SHORT_FIGHT)))
      {
         if (!IS_NPC(victim) && IS_SET(victim->pcdata->pflags, PFLAG_BLIND_PLAYER))
            ...
            act("@@NYour @@dcloak@@N flares, and shrouds $N with @@dmisery@@N!!!", victim, NULL, ch,
                TO_CHAR);
      }
   ...
   if ((is_affected(victim, skill_lookup("cloak:drain"))) && (ch != victim) &&
       (number_range(0, 99) < 35))
   {
      int drain_damage;
...
bool cloak_oathbreaker_avoids_spell_damage(int dam, int element, bool has_cloak, int pseudo_level,
                                           int roll_percent)
{
   if (!has_cloak || dam <= 0 || IS_SET(element, ELE_PHYSICAL) || IS_SET(element, NO_REFLECT) ||
       IS_SET(element, NO_ABSORB))
      return FALSE;
