...
/* ------------------------------------------------------------------ */
/* Import data/ files (bans, socials, rulers, sysdata, etc.)           */
/* ------------------------------------------------------------------ */

static int import_data_file(PGconn *conn, const char *data_dir, const char *filename,
                            const char *table, const char *col)
{
   ...
   {
      free(raw);
      fprintf(stderr, "import_to_db: skipping %s (not found or empty)\n", path);
      return 0;
   }

   char sql[512];
   snprintf(sql, sizeof(sql), "UPDATE %s SET %s = $1 WHERE id = 1", table, col);
...
int main(int argc, char **argv)
{
   ...
   if (do_data)
   {
      /* Store raw flat-file content in sysdata for reference. */
      total += import_data_file(conn, "data", "bans.lst", "sysdata", "id");
      printf("  data: bans.lst logged\n");
   }
