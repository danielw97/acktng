...
const struct stance_app_type stance_app[MAX_STANCE] = {
    {"Adventurer", 0, 0, 0, 0, 0, 0, 0,
     MORTAL, -1, -1, -1, -1},
    /* Mortal stances */
    {"Magi", 5, -3, -3, 0, 0, 2, STANCE_MULTI_CAST | STANCE_NO_HIT,
    ...
    {"Warden", 0, 1, 0, 0, 0, 0, 0,
     MORTAL, CLASS_WAR, 60, -1, -1, CLASS_WAR},
    {"Cipher", -1, 0, 1, 0, 0, 0, 0,
     MORTAL, CLASS_CIP, 60, -1, -1, CLASS_CIP},
    {"Pugilist", 0, 0, 0, 1, 0, 0, 0,
     MORTAL, CLASS_PUG, 60, -1, -1, CLASS_PUG},
    /* Remort stances */
    {"Assassin", -3, 0, 3, 1, 0, 0, 0,
     REMORT, CLASS_ASS, 1, -1, -1, -1},
    {"Ninja", 0, 0, 0, 2, 0, 0, STANCE_NINJA,
     REMORT, CLASS_ASS, 30, -1, -1, -1},
    {"Warlock", -2, 0, 2, 1, 0, 2, 0,
     REMORT, CLASS_WLK, 1, -1, -1, -1},
    {"Priest", -3, 0, 0, 0, 3, 0, 0,
     REMORT, CLASS_PRI, 1, -1, -1, -1},
    {"Paladin", 0, 3, 3, 1, 2, 0, 0,
     REMORT, CLASS_PAL, 1, -1, -1, -1},
    {"Sorcerer", 2, -3, -3, 0, -2, 5, STANCE_DUAL_CAST | STANCE_NO_HIT,
     REMORT, CLASS_SOR, 1, -1, -1, -1},
    {"Wizard", 2, -3, -3, 0, -2, 5, STANCE_DUAL_CAST | STANCE_NO_HIT,
     REMORT, CLASS_WIZ, 1, -1, -1, -1},
    {"Knight", -3, 2, 2, 1, 0, 0, 0,
     REMORT, CLASS_KNI, 1, -1, -1, -1},
    {"Swordsman", -3, 2, 2, 1, 0, 0, 0,
     REMORT, CLASS_GMA, 1, CLASS_KIN, 1, -1},
    {"Necromancer", 2, -3, -3, 0, 0, 5, STANCE_DUAL_CAST | STANCE_NO_HIT,
...
bool is_legal_stance(CHAR_DATA *ch, int stance)
{
    ...
    {
        {
            if (stance_app[stance].class_index > -1 && ch->class_level[MAX_CLASS + MAX_REMORT + stance_app[stance].class_index] < stance_app[stance].class_level)
                legal_stance = FALSE;

            if (stance_app[stance].class_index2 > -1 && ch->class_level[MAX_CLASS + MAX_REMORT + stance_app[stance].class_index2] < stance_app[stance].class_level2)
                legal_stance = FALSE;

            if(IS_SET(stance_app[stance].specials, STANCE_EITHER_CLASS) && stance_app[stance].class_index2 > -1 &&
               ch->class_level[MAX_CLASS + MAX_REMORT + stance_app[stance].class_index] >= stance_app[stance].class_level)
                legal_stance = TRUE;

            if(IS_SET(stance_app[stance].specials, STANCE_EITHER_CLASS) && stance_app[stance].class_index2 > -1 &&
               ch->class_level[MAX_CLASS + MAX_REMORT + stance_app[stance].class_index2] >= stance_app[stance].class_level2)
                legal_stance = TRUE;
        }
    }
    ...
    {
        {
            if (stance_app[stance].class_index > -1 && ch->class_level[MAX_CLASS + stance_app[stance].class_index] < stance_app[stance].class_level)
                legal_stance = FALSE;

            if (stance_app[stance].class_index2 > -1 && ch->class_level[MAX_CLASS + stance_app[stance].class_index2] < stance_app[stance].class_level2)
                legal_stance = FALSE;

            if(IS_SET(stance_app[stance].specials, STANCE_EITHER_CLASS) && stance_app[stance].class_index2 > -1 &&
               ch->class_level[MAX_CLASS + stance_app[stance].class_index] >= stance_app[stance].class_level)
                legal_stance = TRUE;

            if(IS_SET(stance_app[stance].specials, STANCE_EITHER_CLASS) && stance_app[stance].class_index2 > -1 &&
               ch->class_level[MAX_CLASS + stance_app[stance].class_index2] >= stance_app[stance].class_level2)
                legal_stance = TRUE;
        }
    }
