...
bool spec_cinderteeth_warden_cmd(CHAR_DATA *ch)
{
   ...
   if (ch->spec_behavior2 == 0 && ch->max_hit > 0 && ch->hit * 100 / ch->max_hit < 40)
   {
      ch->spec_behavior2 = 1;
      sprintf(buf, "@@W$n straightens with terrible authority — a burning commission\r\n"
   ...
   case 0:
      spell = "dispel magic";
      sprintf(buf, "@@W$n raises $s commission seal and invokes the Compact's\r\n"
                   "arcane-restriction clause — $N's magical protections are stripped!@@N");
      break;

   case 1:
      spell = "chain lightning";
      sprintf(buf, "@@W$n pronounces a warden enforcement citation and fires a\r\n"
                   "bolt of punitive lightning into $N!@@N");
      break;

   case 2:
      spell = "flamestrike";
      sprintf(buf, "@@W$n reads from the penalty register — $N is listed in\r\n"
                   "material violation, and the sentence is flame!@@N");
      break;

   default:
      spell = "harm";
      sprintf(buf, "@@W$n levels a skeletal finger at $N — the warden's authority\r\n"
