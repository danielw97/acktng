...
static void do_dispel_all(CHAR_DATA *ch)
{
   ...
   sprintf(buf, "@@R$n's geological consciousness turns its attention to the intruders —\r\n"
...
static void do_tectonic_release(CHAR_DATA *ch)
{
   ...
   sprintf(buf, "@@R$n releases centuries of tectonic pressure — the caldera floor\r\n"
   ...
   for (vch = ch->in_room->first_person; vch != NULL; vch = vch_next)
   {
      ...
      if (IS_NPC(vch))
         continue;
      calculate_damage(ch, vch, number_range(6000, 9000), gsn_holystrike, ELEMENT_EARTH, TRUE);
   }
}
...
static void do_ember_verdict(CHAR_DATA *ch)
{
   ...
   sprintf(buf, "@@R$n pronounces the EMBER VERDICT — fire, stone, and volcanic wind\r\n"
   ...
   for (vch = ch->in_room->first_person; vch != NULL; vch = vch_next)
   {
      ...
      if (IS_NPC(vch))
         continue;
      calculate_damage(ch, vch, number_range(8000, 12000), gsn_holystrike,
...
bool spec_cinderteeth_patriarch(CHAR_DATA *ch)
{
   ...
   case 2:
      sprintf(buf, "@@R$n's presence deepens — the caldera walls radiate red and\r\n"
                   "the magma chamber below rumbles with building pressure.@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 3:
      sprintf(buf, "@@R$n's attention narrows on $N — the magma surge is almost\r\n"
   ...
   case 4:
   {
      ...
      sprintf(buf, "@@R$n releases the magma surge — fire and volcanic stone\r\n"
                   "erupt from the floor directly beneath $N in a towering column!@@N");
      act(buf, ch, NULL, victim, TO_ROOM);
      calculate_damage(ch, victim, dam, gsn_holystrike, ELEMENT_FIRE | ELEMENT_EARTH, TRUE);
   }
   break;

   case 5:
      sprintf(buf, "@@R$n settles — the caldera vents hiss and the fault-lines\r\n"
   ...
   case 7:
      sprintf(buf, "@@R$n exhales — the caldera's volcanic gas reserves vent in a\r\n"
   ...
   case 8:
   {
      ...
      sprintf(buf, "@@R$n's accumulated centuries press directly against $N's mind —\r\n"
   ...
   case 9:
      sprintf(buf, "@@R$n's entire caldera body glows — the Firsttooth, Secondtooth,\r\n"
                   "and every vent in the Ember Crown begin to pulse in unison.@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 10:
      sprintf(buf, "@@R$n raises $s presence to its full geological scope — fire\r\n"
                   "and earth and volcanic wind are all building toward something.@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 11:
      sprintf(buf, "@@RALL ELEMENTS CONVERGE — $n's geological verdict takes final shape!\r\n"
