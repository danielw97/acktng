...
bool spec_cinderteeth_ventspeaker(CHAR_DATA *ch)
{
   ...
   {
   case 1:
      sprintf(buf, "@@p$n intones in Old Cindertongue — the vent rhythms have named $N\r\n"
   ...
   case 3:
   {
      ...
      sprintf(buf, "@@p$n raises a name-stone and speaks $N's true name into the vent —\r\n"
                   "the sound binds like iron, tearing at $S spirit and shadow!@@N");
      act(buf, ch, NULL, victim, TO_ROOM);
      calculate_damage(ch, victim, dam, gsn_holystrike, ELEMENT_MENTAL | ELEMENT_SHADOW, TRUE);
   }
   break;

   case 5:
      sprintf(buf, "@@p$n exhales the memory of the Ashfall Monsoon — three years\r\n"
      ...
      for (vch = ch->in_room->first_person; vch != NULL; vch = vch_next)
      {
         ...
         if (IS_NPC(vch))
            continue;
         calculate_damage(ch, vch, number_range(3000, 5500), gsn_holystrike,
                          ELEMENT_PHYSICAL | ELEMENT_AIR, TRUE);
      }
      break;

   case 7:
      sprintf(buf, "@@p$n presses both hands against the volcanic floor and communes\r\n"
   ...
   case 8:
   {
      ...
      sprintf(buf, "@@p$n speaks a prophecy with no future in it — the hollow\r\n"
