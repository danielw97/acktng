 ...
 *   round  3 — STRIKE: void-fire blast against primary target (single-target)
...
static void do_void_fire_strike(CHAR_DATA *ch)
{
   ...
   calculate_damage(ch, ch->fighting, dam, gsn_holystrike, ELEMENT_FIRE | ELEMENT_SHADOW, TRUE);
}
...
bool spec_pyramid_black_sun_shard(CHAR_DATA *ch)
{
   ...
   {
   case 1:
      sprintf(buf, "@@R$n begins coiling stolen sunlight into itself — the air turns\r\n"
                   "scorching cold as the eclipse builds toward detonation!@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 3:
      sprintf(buf, "@@RThe Black Sun detonates!  $n unleashes a pillar of void-fire\r\n"
   ...
   case 5:
      sprintf(buf, "@@D$n pulses with cold inverted fire — somewhere far above, the\r\n"
                   "oases shudder as warmth drains downward into the Shard.@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 7:
      sprintf(buf, "@@D$n's void deepens — the sanctum fills with the stench of\r\n"
                   "poisoned springs and drowned palms.  Something vast is exhaling.@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 9:
      sprintf(buf, "@@D$n releases a wave of oasis-death across the entire sanctum —\r\n"
