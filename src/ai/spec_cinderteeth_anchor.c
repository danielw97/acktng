...
static void do_resonance_pulse_aoe(CHAR_DATA *ch)
{
   ...
   sprintf(buf, "@@l$n's crystal conduits discharge in unison — a RESONANCE PULSE\r\n"
   ...
   for (vch = ch->in_room->first_person; vch != NULL; vch = vch_next)
   {
      ...
      if (IS_NPC(vch))
         continue;
      calculate_damage(ch, vch, number_range(4000, 7000), gsn_holystrike,
...
static void do_anchor_overload_aoe(CHAR_DATA *ch)
{
   ...
   sprintf(buf, "@@l$n's crystal lattice reaches critical saturation — an ANCHOR\r\n"
   ...
   for (vch = ch->in_room->first_person; vch != NULL; vch = vch_next)
   {
      ...
      if (IS_NPC(vch))
         continue;
      calculate_damage(ch, vch, number_range(6000, 9000), gsn_holystrike, ELEMENT_MENTAL, TRUE);
   }
}
...
bool spec_cinderteeth_anchor(CHAR_DATA *ch)
{
   ...
   case 1:
      ch->spec_behavior2 = 0;
      sprintf(buf, "@@l$n's crystal conduits begin to resonate — standing waves\r\n"
                   "build in the chamber walls with a low, bone-deep hum!@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 2:
      sprintf(buf, "@@l$n's resonance charge intensifies — the hum becomes painful!\r\n"
                   "@@WStrike with @@yEARTH@@W or @@yPHYSICAL@@W force to ground the conduit!@@N");
      act(buf, ch, NULL, NULL, TO_ROOM);
      break;

   case 3:
      if (IS_SET(ch->spec_behavior2, ELEMENT_EARTH) || IS_SET(ch->spec_behavior2, ELEMENT_PHYSICAL))
      {
         sprintf(buf, "@@yYour earthen/physical blow disrupts $n's crystalline conduit!\r\n"
   ...
   case 5:
   {
      ...
      sprintf(buf, "@@l$n focuses $s standing-wave output into a single conduit\r\n"
                   "and fires a piercing resonance lance directly into $N!@@N");
      act(buf, ch, NULL, victim, TO_ROOM);
      calculate_damage(ch, victim, dam, gsn_holystrike, ELEMENT_MENTAL | ELEMENT_AIR, TRUE);
   }
   break;
   ...
   case 7:
   {
      ...
      sprintf(buf, "@@l$n triggers a structural discharge — a deep seismic shudder\r\n"
