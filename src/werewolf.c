...
void do_tribe(CHAR_DATA *ch, char *argument)
{
   ...
   if ((arg1[0] == '\0') || (ch->pcdata->generation > 1))
   {
      ...
      else
      {
         ...
         for (d = first_desc; d != NULL; d = d->next)
         {
            if (d->connected == CON_PLAYING && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room != NULL && IS_WOLF(victim) && !IS_IMMORTAL(victim))
            {
   ...
   else /* multiplexed use of tribe  */
   {
      ...
      if ((IS_NPC(victim)) || (!IS_WOLF(victim)) || (victim->pcdata->vamp_bloodline != ch->pcdata->vamp_bloodline))
      {
