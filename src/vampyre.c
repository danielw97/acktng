...
void do_family(CHAR_DATA *ch, char *argument)
{
   ...
   if (get_trust(ch) == 85)
   {
      for (index = 0; index <= 4; index++)
      {
         ...
         {
            if (d->connected == CON_PLAYING && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room != NULL)
            {
   ...
   else
   {
      ...
      for (d = first_desc; d != NULL; d = d->next)
      {
         if (d->connected == CON_PLAYING && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room != NULL && IS_VAMP(victim) && !IS_IMMORTAL(victim))
         {
...
bool spell_blood_walk(int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj)
{
   ...
   if ((victim = get_char_world(ch, target_name)) == NULL || victim == ch || !IS_NPC(victim) || victim->in_room == NULL || IS_SET(victim->in_room->room_flags, ROOM_PRIVATE) || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY) || IS_SET(victim->in_room->room_flags, ROOM_SAFE) || IS_SET(victim->act, PLR_NOBLOOD) || IS_SET(victim->in_room->room_flags, ROOM_NOBLOODWALK) || ((get_psuedo_level(victim) - get_psuedo_level(ch)) > 20))
   {
