...
static bool load_static_quest_file(const char *path, int id)
{
   ...
   tpl.max_level = 170;
   if (sscanf(line, "%d %d %d %d %d %d %d %d %d %d %*d", &tpl.prerequisite_static_id, &tpl.type,
   ...
   {
      tpl.reward_exp = 0;
      if (sscanf(line, "%d %d %d %d %d %d %d %d %d %*d", &tpl.prerequisite_static_id, &tpl.type,
...
void quest_load_static_templates(void)
{
   ...
   snprintf(buf, sizeof(buf), "Loaded %d static quest template%s from %s.", static_quest_count,
            static_quest_count == 1 ? "" : "s", QUEST_STATIC_DIR);
   log_string(buf);
}
...
/*
 * Raw exp reward for a static quest: 3x the mob_base for a mob at max_level.
...
static bool static_quest_has_boss_target(const STATIC_PROP_TEMPLATE *tpl)
{
   int i;
   if (tpl == NULL)
      return FALSE;
...
static void show_reward_preview(CHAR_DATA *ch, QUEST_DATA *prop)
{
   ...
   const STATIC_PROP_TEMPLATE *tpl =
       prop->quest_static_id >= 0 ? find_static_quest_template(prop->quest_static_id) : NULL;
   int gold =
       prop->quest_static_id >= 0 ? prop->quest_reward_gold : quest_gold(get_psuedo_level(ch));
   ...
   {
      exp = calc_static_quest_exp(tpl->max_level, static_quest_has_boss_target(tpl),
...
void quest_complete(CHAR_DATA *ch, CHAR_DATA *postman)
{
   ...
   for (slot = 0; slot < QUEST_MAX_QUESTS; slot++)
   {
      ...
      else if (prop->quest_static_id >= 0 && tpl != NULL)
      {
         exp_reward = calc_static_quest_exp(tpl->max_level, static_quest_has_boss_target(tpl),
                                            tpl->type == QUEST_TYPE_CARTOGRAPHY);
         if (is_adept(ch))
            exp_reward /= 1000;
      ...
      qp_reward =
          prop->quest_static_id >= 0 ? prop->quest_reward_qp : quest_qp(get_psuedo_level(ch));
   ...
   if (!quest_active(ch))
      ...
      send_to_char("You cannot turn that in here yet. Dynamic quests need a postman; static quests "
